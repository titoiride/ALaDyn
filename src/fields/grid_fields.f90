  ...
  do ic=ic1,ic2
   do k=kz1,kz2
    do j=jy1,jy2
     do i=i1,n1p
      ii=i-2
      ww0(ii,1)=ef(i,j,k,ic)-aphx_exp*(ef(i+1,j,k,ic)-ef(i-1,j,k,ic))
     end do
     do i=i1,n1p
      ii=i-2
  ...
  do ic=ic1,ic2
   do k=kz1,kz2
    do j=jy1,jy2
     do i=i1,n1p
      ii=i-2
      ww0(ii,1)=ef(i,j,k,ic)-aphx_impl*(ef(i+1,j,k,ic)-ef(i-1,j,k,ic))
     end do
...
!==================================
 ...
 do k=kz1,kz2
  do j=jy1,jy2
   ...
   envg(i+1,j,k,2)=dx_inv*(2.*envg(i,j,k,1)-3.*envg(i-1,j,k,1)+envg(i-2,j,k,1))
  end do
 end do
 if(der_ord==4)then
  do k=kz1,kz2
 ...
 if(pe0x)then
  if(ibx <2)then
   ...
   do k=kz1,kz2
    do j=jy1,jy2
     ef(ix1-1,j,k,nfield)=-&
      (2.*ef(ix1,j,k,2)+(1.-aphx)*ef(ix1,j,k,nfield))/(1.+aphx)
    end do
   end do
   if(nfield>3)then
    !==========================
    ...
    do k=kz1,kz2
     do j=jy1,jy2
      ef(ix1-1,j,k,5)=&
       (2.*ef(ix1,j,k,3)-(1.-aphx)*ef(ix1,j,k,5))/(1.+aphx)
     end do
    end do
   endif
  endif
 ...
 if(iby < 2)then
  if(pe0y)then
   ...
   case(0)
    ...
    do k=kz1,kz2
     do i=ix1,ix2
      ef(i,jy1-1,k,nfield)=2.*ef(i,jy1,k,1)-(1.-aphy)*ef(i,jy1,k,nfield)
    ...
    if(nfield>3)then
     ...
     do k=kz1,kz2
      do i=ix1,ix2
       ef(i,jy1-1,k,4)=-2.*ef(i,jy1,k,3)-(1.-aphy)*ef(i,jy1,k,4)
 ...
 if(ibz <2)then
  if(pe0z)then
   ...
   case(0)
    ...
    do j=jy1,jy2
     do i=ix1,ix2
      ef(i,j,kz1-1,4)=2.*ef(i,j,kz1,2)-(1.-aphz)*ef(i,j,kz1,4)
      ef(i,j,kz1-1,4)=ef(i,j,kz1-1,4)/(1.+aphz)
      ef(i,j,kz1-1,5)=-2.*ef(i,j,kz1,1)-(1.-aphz)*ef(i,j,kz1,5)
  ...
  if(pe1x)then
   ...
   case(0)
    ...
    do k=kz1,kz2
     do j=jy1,jy2
      ef(ix2+1,j,k,2)=&
       (2.*ef(ix2,j,k,nfield)-(1.-aphx)*ef(ix2,j,k,2))/(1.+aphx)
     end do
    end do
   ...
   if(nfield>3)then
    ...
    case(0)
     do k=kz1,kz2
      do j=jy1,jy2
       ef(ix2+1,j,k,3)=-&
        (2.*ef(ix2,j,k,5)+(1.-aphx)*ef(ix2,j,k,3))/(1.+aphx)
      end do
     end do
    case(1)   !reflecting
     do k=kz1,kz2
  ...
  if(pe1y)then
   ...
   case(0)
    ...
    do k=kz1,kz2
     do i=ix1,ix2
      ef(i,jy2+1,k,1)=-&
       (2.*ef(i,jy2,k,nfield)+(1.-aphy)*ef(i,jy2,k,1))/(1.+aphy)
     end do
    end do
    if(nfield>3)then
     !++++++++++++++++++++++++++++++++++++++ (Bz,Ex)
     ...
     do k=kz1,kz2
      do i=ix1,ix2
       ef(i,jy2+1,k,3)=&
        (2.*ef(i,jy2,k,4)-(1.-aphy)*ef(i,jy2,k,3))/(1.+aphy)
      end do
     end do
    endif
  ...
  if(pe1z)then
   ...
   case(0)
    ...
    do j=jy1,jy2
     do i=ix1,ix2
      ef(i,j,kz2+1,1)=&
       (2.*ef(i,j,kz2,5)-(1.-aphz)*ef(i,j,kz2,1))/(1.+aphz)
      ef(i,j,kz2+1,2)=-&
       (2.*ef(i,j,kz2,4)+(1.-aphz)*ef(i,j,kz2,2))/(1.+aphz)
     end do
    end do
...
! momenta-density
  do k=kz1,kz2
   ...
   call weno3_nc(fcomp+1,ix1,ix2,xl_bd,xr_bd) 
