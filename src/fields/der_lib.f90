 ...
 contains
 !==========================================
 subroutine env_matrix_inv(jcr,lk0,dhx,b_diag,i1,n1p,j1,n2p,k1,n3p)
 ...
 do k=k1,n3p
  do j=j1,n2p
   ...
   do i=i1+1,n1p-1
    ii=ii+1
    ww0(ii,1)=0.5*dhx*(jcr(i+1,j,k,1)-jcr(i-1,j,k,1)) +lk0*jcr(i,j,k,2)
    ww0(ii,2)=0.5*dhx*(jcr(i+1,j,k,2)-jcr(i-1,j,k,2)) -lk0*jcr(i,j,k,1)
   end do
   ii=ii+1
   i=n1p
   ww0(ii,1)=dhx*(jcr(i,j,k,1)-jcr(i-1,j,k,1))+lk0*jc(i,j,k,2)
   ww0(ii,2)=dhx*(jcr,j,k,2)-jcr(i-1,j,k,2))-lk0*jcr(i,j,k,1)
  end do
 end do
 do ic=1,2
  do k=k1,n3p
