...
&
 ...
 ! adds currents
 do ik=1,curr_ndim
  ef(ix1:ix2,jy1:jy2,kz1:kz2,ik)=ef(ix1:ix2,jy1:jy2,kz1:kz2,ik)-&
                           ompe*curr(ix1:ix2,jy1:jy2,kz1:kz2,ik)
 end do
 if(Comoving)then
  ...
  do ic=1,nb_laser
   if(lp_in(ic) < x_left.and.lp_end(ic)>= xm)then
    ...
    if(model_id<3) call inflow_lp_fields(&
              ebf,lp_amp,tnew,t0_lp,w0_x,w0_y,xf_loc(ic),oml,wmodel_id,&
              ix1,jy1,jy2,kz1,kz2)
    if(model_id==3)call inflow_cp_fields(&
              ebf,lp_amp,tnew,t0_lp,w0_x,w0_y,xf_loc(ic),wmodel_id,&
              ix1,jy1,jy2,kz1,kz2)
   endif
  ...
  if(Two_color)then
   if(lp_ionz_in < x_left.and.lp_ionz_end >=xm)then
    ...
    call inflow_lp_fields(&
              ebf,lp1_amp,tnew,t1_lp,w1_x,w1_y,xf1,om1,model_id,&
              ix1,jy1,jy2,kz1,kz2)
   endif
