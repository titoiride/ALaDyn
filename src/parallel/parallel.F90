...
!
 ...
 if(sr)then
  call mpi_send(buff(1),lenw,mpi_sd,ipe,tag, &
   comm_col(dir),error)
 else
  call mpi_recv(buff(1),lenw,mpi_sd,ipe,tag, &
   comm_col(dir),status,error)
 endif
 ...
 call mpi_sendrecv(buff1(1),lenws,mpi_sd,pes,tag, &
  buff2(1),lenws,mpi_sd,per,tag, &
  comm_col(dir),status,error)
