...
!*****************************************************************************************************!
 ...
 contains
  ...
  subroutine set_local_2d_positions(pt_loc, n1, np)
   ...
   !=========================
   do n = 1, np
    pt_loc(n, 1) = dx_inv*(pt_loc(n,1)-xmn) + 1
   end do
   if (n1==0) return
   if (n_str==0) then
    do n = 1, np
     pt_loc(n, 2) = dy_inv*(pt_loc(n,2)-ymn) + 1
    end do
   else
   ...
   do n = 1, np
    pt_loc(n, 3) = dx_inv*(pt_loc(n,3)-xmn) + 1
   end do
   if (n_str==0) then
    do n = 1, np
     pt_loc(n, 4) = dy_inv*(pt_loc(n,4)-ymn) + 1
    end do
   else
  ...
  subroutine set_local_3d_positions(pt_loc, n1, np)
   ...
   if (n_str==0) then
    do n = 1, np
     pt_loc(n, 2) = dy_inv*(pt_loc(n,2)-ymn) + 1
     pt_loc(n, 3) = dz_inv*(pt_loc(n,3)-zmn) + 1
    end do
   else
   ...
   if (n1==1) return
   do n = 1, np
    pt_loc(n, 4) = dx_inv*(pt_loc(n,4)-xmn) + 1
   end do
   if (n_str==0) then
    do n = 1, np
     pt_loc(n, 5) = dy_inv*(pt_loc(n,5)-ymn) + 1
     pt_loc(n, 6) = dz_inv*(pt_loc(n,6)-zmn) + 1
    end do
   else
